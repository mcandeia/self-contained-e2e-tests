...
def k6_local_resource(run_grafana=False, add_prometheus=True):
    ...
    k8s_kind('K6', image_json_path='{.spec.runner.image}', pod_readiness='wait')

def k6_test_resource(
        name='k6-test',
        namespace='',
        from_file=None,
        parallelism=1,
        prometheus_url='http://prometheus-kube-prometheus-prometheus.default.svc.cluster.local:9090/api/v1/write',
    ...
    k8s_yaml(encode_yaml({
        ...
        'spec': {
            ...
            'arguments': '--include-system-env-vars' + (' -o output-prometheus-remote' if prometheus_url != '' else ''),
            'runner': {
                'image': 'k6-custom',
                'metadata': {
                    'annotations': {
                ...
                'env': (runner_env + [{ 'name': 'K6_PROMETHEUS_REMOTE_URL', 'value': prometheus_url}, { 'name': 'K6_PROMETHEUS_USER', 'value': prometheus_user}, { 'name': 'K6_PROMETHEUS_PASSWORD', 'value': prometheus_password}]) if prometheus_url != '' else runner_env
            }
        }
    }))

    k8s_resource(name, resource_deps=resource_deps + [test_file_resource, k6_local_resource_name])
